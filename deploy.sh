...
set -x
yarn workspace @platonist/api run stop -- --name api-"$NODE_ENV" || echo "Process was not running."
set +x
set -x
yarn workspace @platonist/api run serve -- --name api-"$NODE_ENV" --log pm2.log
set +x
# END # ********************************************************************* #
